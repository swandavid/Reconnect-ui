...
export default function Rating({ toggleTask }) {
    ...
    return (
        <div tw="grid place-items-center md:flex md:flex-row" onClick={ toggleTask }>
            ...
            { hasRating ? (
                null
            ) : (<button tw="font-display font-semibold p-2 bg-white text-xs lg:text-base text-green-800 hover:bg-gray-200 m-2 rounded-lg" onClick={()=>{setHasRating(true)}}>Submit</button>)
            }
        </div>
    );
};